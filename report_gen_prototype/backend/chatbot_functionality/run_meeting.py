...
"""
Top bar:
Home(command line interface mirror) Agents(dropdown) Leads(dropdown)
MODELS/views:
lead: name, descr, reports
reports:name, date, params, lead
agents: 

Home:
__________________________________________________________________________________________
Descr: You are a client describing a report you want created on a specific topic. 
Create/use previous project leads(leads just represent a shared chat log knowledge base
ie, if you use a previous lead over creating a new one, that lead will create a team
that drawsinfo from previous report gen conversations. Create a new lead if you want to start fresh.


If new lead created:
Name:(identifier not used in conv)
Descr: Description to be shown when you choose

Once team chosen:
Task: text box
Report name:(optional, can be generated)
Expectations: text box
Context: choose upload file or text box
Cycles: number
Report guidelines: text
Method: Text(with description)
Temperature: num
Engine: dropdown

Generate button.
Loading animation(with description of which phase it's in, possible display some info/scrolling chat log)

At end:
generate two outputs. Report(download as pdf option), full chat log(download option)
Entry generated in django db: lead, report name, all above params, full chat log, agents used, datetime(pkey)
...
Reports generated by this lead:
Report name - Date (hyperlink)
...
Report name - Date
Parameters used:
Agents used(hyperlink to each one)
Full chat log:
Report pdf
...
role
expertise
Context uploaded(store pdfs to each paper, or just link/name?)
Button: upload new paper
