...
import json
from graphrag_test.agent_framework.Agent import ollama_engine
import faiss
...
class KnowledgeBase():
    def __init__(self, vector_store_location, engine):
    ...
    def upload_knowledge_1(self, text, source_name):
            ...
            for i in chunks:
                ...
                raw_info = str(i)
                #raw_info = self.engine.generate(summarize_query)
                vector_docs.append(Document(
                    page_content= raw_info,
                    metadate={"source": source_name}
                ))
                print(raw_info)
